...
Example web app built with Node.js, Express, and SQLite.

## Cloning the repository on the EC2 instance
...
You can now run the setup scripts located in the `scripts` directory to
install necessary packages and configure the server.

## Initial Configuration Scripts

The scripts listed below should be run in the given order to set up a new EC2
instance for hosting the Node.js application. You only need to run these scripts
**once** during the initial setup.

- `install-packages.sh`: A script to install necessary packages for the project.
  -  **IMPORTANT**: You ** MUST exit** the terminal and re-login after running
    this script to refresh your environment for nvm to work. If you know how to
    source your profile file instead of re-logging in, that is also acceptable.
...
- If you ask for help and say "I ran the setup script and it didn't work", I
  will ask you to run through the commands manually anyway. So just do that
  first to save time so we can focus on fixing the actual issue.
